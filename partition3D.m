function [partsCell,partsCellLen,parentCell]=partition3D(myImage,myMask,sigmai,sigmax_constant,voxelSize)

% 3D SHARP partitioning algorithm
%
% Inputs:
% myImage: 3D double-valued image
% myMask: 3D image. Values of 1 inside tumor, 0 outside tumor.
% sigmai: intensity sensitivity constant for Normalized Cut
% sigmax_constant: distance sensitivity constant for Normalized Cut
% voxelSize: 1x3 array of voxel dimensions in mm
...
numSamples=100;
numKmeansReps=10;
[dimx, dimy, dimz]=size(myImage);
whereTumor=find(myMask>0);
whereTumorInv=zeros(dimx*dimy*dimz,1,'uint32');
whereTumorInv(whereTumor)=1:size(whereTumor,1);
[whereTumorX,whereTumorY,whereTumorZ]=ind2sub([dimx dimy dimz],whereTumor);
myFilter=zeros(3,3,3);
myFilter(2,2,2)=1; myFilter(2,2,1)=1; myFilter(2,2,3)=1;
myFilter(1,2,2)=1; myFilter(2,1,2)=1; myFilter(3,2,2)=1; myFilter(2,3,2)=1;
...
while needsDivideCount && level <= maxLevel-1
    ...
    for currentPart=1:toDivideCount
        ...
        sigmax=sigmax_constant*(size(labelsIn,2)^(1/3))*prod(voxelSize);
        ...
        if size(labelsIn,2) == 2
            ...
            if size(labelsIn,2) <= numSamples
                ...
                [z1, z2]=meshgrid(whereTumorZ(labelsIn(perm)));
                [int1, int2]=meshgrid(myImage(whereTumor(labelsIn(perm))));
                distMat=exp(-((( ((x2-x1)*voxelSize(1)).^2 + ((y2-y1)*voxelSize(2)).^2 + ((z2-z1)*voxelSize(3)).^2 )/sigmax^2) + ((int1-int2)/sigmai).^2));
                [V,~]=ncut(distMat);
            else
                ...
                [z1, z2]=meshgrid(whereTumorZ(labelsIn(perm)), whereTumorZ(labelsIn(perm(1:numSamples))));
            ...
            myCut=zeros(dimx,dimy,dimz); % find # of connected components (want to be 2)
            myCut(whereTumor(labelsIn))=labels;
            myCutExpanded=zeros(dimx+8,dimy+8,dimz+8);
            myCutExpanded(5:end-4,5:end-4,5:end-4)=myCut;
            CC1=bwconncomp(myCutExpanded==1,6);
            CC2=bwconncomp(myCutExpanded==2,6);
            ...
            if CC1.NumObjects+CC2.NumObjects > 2 % one partition is discontinuous
                    ...
                    if numObjects(whichPart) > 1
                        ...
                        for whichObject=1:CC.NumObjects
                            if whichObject ~= largestPartID
                                ...
                                tempMask=objectExpanded & otherMask;
                                if max(tempMask(:))
                ...
                bestCut=myCutExpanded(5:end-4,5:end-4,5:end-4);
